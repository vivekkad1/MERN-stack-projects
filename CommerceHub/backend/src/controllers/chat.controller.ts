...
export const handleChat = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const products = await Product.find({ isActive: true }).select('title basePrice category stock brand').populate('category', 'name');
    
    // Convert to a minimal string to save tokens
    const productCatalog = products.map(p => 
      `- ${p.title} (Price: ₹${p.basePrice}, Category: ${(p.category as any)?.name || 'N/A'}, Stock: ${p.stock > 0 ? 'In Stock' : 'Out of Stock'})`
    ).join('\n');
